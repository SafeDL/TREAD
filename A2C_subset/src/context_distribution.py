...
class TailContextDistribution:
    ...
    def __post_init__(self) -> None:
        ...
        if self.event_type == "cut_in":
            self.time_to_cross_sua2crt = np.unique(self.marginal_values[:, 8])
        else:
            self.time_to_cross_sua2crt = np.asarray([], dtype=np.float64)
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if self.event_type == "cut_in":
            ...
            if self.time_to_cross_sua2crt.size:
                nearest = int(np.argmin(np.abs(self.time_to_cross_sua2crt - feature[8])))
                feature[8] = float(self.time_to_cross_sua2crt[nearest])
