...
from adversaray.src.risk_utils import interaction_metrics_from_states, write_csv, write_json
...
def _selected_split_indices(raw: dict[str, np.ndarray], split: str, max_contexts: int, seed: int) -> np.ndarray:
    idx = np.where(raw["split_index"] == SPLIT_TO_INDEX[str(split)])[0].astype(np.int64)
    if idx.size == 0:
        raise RuntimeError(f"No contexts found for split '{split}'")
    if int(max_contexts) > 0 and idx.size > int(max_contexts):
        rng = np.random.default_rng(int(seed))
        idx = np.sort(rng.choice(idx, size=int(max_contexts), replace=False)).astype(np.int64)
    return idx
...
def build_tail_scores(
    *,
    ...
    if not sources:
        raise ValueError("SCRIPT_DEFAULTS['source'] must include at least one source")
    ...
    if "initial_context" in sources:
        _add_components(
            raw_accum,
            _raw_components(initial_min_rss, initial_min_ttc, initial_min_gap, initial_metrics["initial_closing_speed"], eps=float(eps)),
        )
    ...
    score = (w_rss * rss_component + w_ttc * ttc_component + w_gap * gap_component + w_dv * dv_component).astype(np.float32)
    threshold = float(np.quantile(score[np.isfinite(score)], float(tail_quantile)))
...
def export_tail_contexts(
    *,
    ...
    if missing_tail:
        raise KeyError(f"{tail_path} is missing required arrays: {missing_tail}")
    ...
    if not np.any(mask):
        raise RuntimeError(f"No natural tail contexts found for split '{split}' at quantile {tail_quantile}")
    ...
    logger.info("Wrote %d natural tail contexts to %s", len(selected), output_path)
    return output_path
