...
class FrozenDiffusionSampler:
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any], *, config_dir: str | Path | None = None) -> "FrozenDiffusionSampler":
    ...
    def _randn(
        ...
        *,
        ...
        if isinstance(generators, list):
            ...
            return torch.cat([torch.randn(1, *shape[1:], **kwargs, generator=generator) for generator in generators], dim=0)
        return torch.randn(*shape, **kwargs, generator=generators)

    def _initial_noise(self, batch_size: int, *, generators: torch.Generator | list[torch.Generator] | None) -> torch.Tensor:
    ...
    def sample(
        ...
        *,
        ...
        kin = integrate_following_actions_torch(raw_actions, raw_context, ego_length, adv_length, self.prior.schema, self.config)
