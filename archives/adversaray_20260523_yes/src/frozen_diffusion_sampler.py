...
class FrozenDiffusionSampler:
    ...
    def _risk_guided_reverse_step(
        ...
        *,
        ...
        with torch.enable_grad():
            ...
            risk, _risk_diag = compute_king_risk(
                kin,
                self.config,
                reference_kin,
            )
    ...
    def sample(
        ...
        *,
        ...
        if bool(risk_tilted):
            ...
            if reference_actions.shape[0] != context_states.shape[0]:
                raise ValueError(
                    ...
                    f"got {reference_actions.shape[0]} and "
                    f"{context_states.shape[0]}"
                )
        ...
        risk_ref = reference_kin if reference_kin is not None else kin
        risk_obj, risk_diag = compute_king_risk(kin, self.config, risk_ref)
