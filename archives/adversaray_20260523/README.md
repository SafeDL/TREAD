...
并调用 highway-env `IDMVehicle` 得到 detached ego trace。风险由 TTC /
DRAC / gap、相对 prior 的 delta RSS 和 RSS improper response 组成，梯度只
回到 adversary 控制序列，ego 响应不反传。
...
- `src/context_utils.py`：context NPZ 读取和单条 context 包装。
...
`lead_max_brake=7.625`。raw RSS margin 只做诊断；优化和闭环评分使用
...
默认 KING risk 尺度位于 `king_gradient` block：`ttc_scale=1.0`、
`drac_scale=5.0`、`gap_scale=20.0`。
`sample_king_guided_diffusion.py` 日志中的 `risk before -> after` 使用
highway-env IDM ego trace；`evaluate_king_guided_samples.py` 日志中的
`closed-loop risk` 是重新 rollout 后由 collision、gap、TTC、DRAC 和
delta RSS 等指标组成的验证风险。二者都不是碰撞概率。
