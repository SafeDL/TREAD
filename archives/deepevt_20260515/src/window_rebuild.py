...
actor1 = target) 以及在前缀之后的未来事件段内重新计算的响应风险
``future_risk_score``。
...
def get_event_prediction_frames(
    event_row: pd.Series,
    config: dict,
) -> tuple[np.ndarray, np.ndarray]:
    """Return (prefix_frames, future_risk_frames) for event-level prediction.

    The preferred DeepEVT task is now:

    * input/context: the first ``prefix.prefix_steps`` frames of the event;
    * target: risk over frames strictly after that prefix until event end.

    This makes the model a conditional prior / search guide: it observes the
    early ego-target interaction and predicts future risk quantiles for the
    remaining naturalistic driving event.
    """
    prefix_steps = int(config.get("prefix", {}).get("prefix_steps", 12))
    if prefix_steps < 1:
        raise ValueError(f"prefix.prefix_steps must be >= 1, got {prefix_steps}")

    target_cfg = config.get("risk_target", {})
    start_key = str(target_cfg.get("event_start_key", "start_frame"))
    end_key = str(target_cfg.get("event_end_key", "end_frame"))
    start = event_row.get(start_key)
    end = event_row.get(end_key)
    if pd.isna(start):
        start = event_row.get("risk_window_start_frame")
    if pd.isna(end):
        end = event_row.get("risk_window_end_frame")
    if pd.isna(start) or pd.isna(end):
        anchor = int(event_row["anchor_frame"])
        window = get_analysis_frames(event_row, config)
        start_i = int(window[0])
        end_i = int(window[-1])
        logger.debug(
            "[%s] event bounds missing; fallback around anchor=%d",
            event_row.get("event_id", "?"), anchor,
        )
    else:
        start_i = int(start)
        end_i = int(end)

    prefix_start = start_i
    prefix_end = prefix_start + prefix_steps - 1
    risk_start = prefix_end + 1
    if bool(target_cfg.get("include_prefix_end_frame", False)):
        risk_start = prefix_end
    prefix_frames = np.arange(prefix_start, prefix_end + 1, dtype=np.int64)
    risk_frames = np.arange(risk_start, end_i + 1, dtype=np.int64)
    return prefix_frames, risk_frames
...
def recompute_window_risk(
    ...
    """在固定 risk window 内重新计算 risk_score。"""
...
def rebuild_event_window(
    ...
    target_mode = str(config.get("risk_target", {}).get("mode", "future_after_prefix"))
    if target_mode == "future_after_prefix":
        prefix_frames, risk_frames = get_event_prediction_frames(event_row, config)
    else:
    ...
    min_future_steps = int(config.get("risk_target", {}).get("min_future_steps", 5))
    if len(risk_frames) < min_future_steps:
        logger.debug(
            "[%s] future risk segment too short: expected >=%d got %d",
            event_row.get("event_id", "?"), min_future_steps, len(risk_frames),
        )
        return None
    ...
    source = (
        "future_after_prefix"
        if target_mode == "future_after_prefix"
        else ("risk_window" if pd.notna(event_row.get("risk_window_start_frame")) else "anchor_window")
    )
    return WindowSample(
