...
from .negative_generators import (
    RANDOM_PERTURB_SOURCE,
    RSS_OVER_GUIDED_SOURCE,
    RULE_BRAKE_SOURCE,
    generate_random_perturb_negatives,
    generate_rss_over_guided_negatives,
...
RSS_SOURCE = RSS_OVER_GUIDED_SOURCE
...
def _resolve_paths(config: dict, config_dir: str | Path | None = None) -> DiscriminatorPaths:
    ...
    output_dir = (base / paths.get("output_dir", "../../../data/adversaray/following/discriminator")).resolve()
...
def _append_block(
    ...
    *,
    ...
    blocks["future_actions"].append(actions.astype(np.float32))
    ...
    blocks["soft_labels"].append(np.full((n,), float(label if soft_label is None else soft_label), dtype=np.float32))
    ...
    *,
    ...
    _append_block(
        ...
        soft_label=float(
            config.get("training", {}).get(
                ...
                config.get("training", {}).get("label_smoothing_positive", 0.9),
            )
        ),
        sample_weight=1.0,
        schema=schema,
    ...
    if bool(neg_cfg.get(RSS_SOURCE, False)):
        ...
        if external is not None:
            ...
            copies = max(1, int(config.get("data", {}).get("rss_over_guided_copies_per_positive", 1)))
            actions = generate_rss_over_guided_negatives(
                raw["actions"][idx],
                raw["context_states"][idx],
                raw["ego_length"][idx],
                raw["adv_length"][idx],
            ...
            _append_block(
                ...
                source_type=RSS_SOURCE,
                label=0.0,
                sample_weight=float(config.get("data", {}).get("rss_over_guided_weight", 1.0)),
                schema=schema,
                config=config,
                repeat_from_index=True,
            )
    ...
    summary = {
        ...
        "rss_over_guided": {
            "enabled": bool(neg_cfg.get(RSS_SOURCE, False)),
            "external_imported": bool(rss_external),
            "local_generated": bool(rss_generated),
        },
        "split_counts": {
