...
- `tread_deepevt/src/window_rebuild.py`：从 highD 原始轨迹与 `events.csv` 重建固定分析窗口，并重算窗口风险。
- `tread_deepevt/src/features.py`：提取不含未来风险泄漏的 initial-context 特征。
- `tread_deepevt/src/data.py`：生成 `dataset.npz`、schema、normalization stats 与 recording 粒度划分。
- `tread_deepevt/src/model.py`：定义 InitialSceneTransformer 与 DeepEVT 参数头。
...
DeepEVT 的核心思想是：给定交通事件的初始场景条件，估计未来固定时间窗内风险得分的条件尾部分布。模型不是只预测一个平均风险，而是预测一个条件阈值 `u`、超阈概率 `p` 以及广义帕累托分布参数 `xi` 和 `beta`，从而进一步得到高分位风险 `q90/q95` 与 Expected Shortfall `ES95`。这适用于交通安全中的长尾问题：少数高风险事件比平均误差更重要。

## 1. 问题定义

### 1.1 样本与目标变量

对每一个 highD 事件样本，`window_rebuild.py` 会重建一个固定长度 analysis window。当前 following 配置为：

- `window_length = 128`
- `target_fps = 25`
- 窗口时间长度约为 `128 / 25 = 5.12 s`
- `prefix_steps = 1`

每个样本的输入包括：

- `prefix_states`: shape 为 `[K, 2, F]`，其中 `K=1`，两个 actor 分别为 ego 和 target，状态通道为 `x, y, vx, vy, ax, ay`。
- `context_features`: following 场景下为 12 维初始上下文特征。
...
当前 following 模型的 12 个 context 特征为：

| 特征 | 含义 |
| --- | --- |
| `ego_vx0` | ego 初始纵向速度 |
| `lead_vx0` | 前车初始纵向速度 |
| `relative_speed_0` | 初始相对速度，`ego_vx0 - lead_vx0` |
| `target_center_x0` | target 初始中心纵向位置 |
| `target_center_y0` | target 初始中心横向位置 |
| `initial_gap` | 初始净纵向间距 |
| `initial_lateral_offset` | 初始横向偏移 |
| `ego_ax0` | ego 初始纵向加速度 |
| `lead_ax0` | 前车初始纵向加速度 |
| `lane_width` | 车道宽度 |
| `dt` | 采样间隔 |
| `horizon_steps` | 预测窗口步数 |

这些特征只来自窗口第 0 帧或场景常量。`features.py` 明确禁止以下未来风险字段进入模型输入：
...
这点非常重要。DeepEVT 的目标是从初始场景条件预测未来风险尾部分布，而不是把未来已经计算出来的风险指标作为输入。
...
### 2.2 InitialSceneTransformer

虽然名称中有 Transformer，但在当前 `prefix_steps=1` 的设置下，它不是时间序列 Transformer，而是一个初始场景 token encoder。其 token 包括：

- 一个可学习的 `cls_token`
- ego/target 两个 actor token
- 12 个 context scalar token

每个 actor 的状态向量经 `actor_proj` 投影到 hidden_dim；每个 context scalar 经 `context_value_proj` 投影到 hidden_dim；再加上 actor type embedding 和 context type embedding。Transformer encoder 输出 `cls_token` 对应的场景表示 `z_scene`。

这种结构的意义是：模型可以通过 self-attention 学习“ego 速度、前车速度、初始间距、相对速度、加速度、道路常量”等因素之间的交互，而不是只做简单拼接回归。
...
DeepEVT 的条件化体现在：`u(x)`、`p(x)`、`xi(x)`、`beta(x)` 都由神经网络根据初始场景 `x` 预测。
...
其中 `{split}` 为 `train` 或 `val`。

### 7.1 优先看 epoch 曲线，而不是 batch 曲线
...
```text
先看 epoch/val，再看 epoch/train，最后才看 batch 曲线。
```

batch 曲线主要用于发现异常尖峰、NaN 或梯度爆炸，不适合直接判断模型好坏。

### 7.2 `meta/stage` 和 `meta/epoch_in_stage`
...
- `meta/stage = 2`：尾部训练。
- `meta/stage = 3`：端到端微调。
- `meta/epoch_in_stage`：当前 stage 内的 epoch 编号。

看曲线时要先对齐 stage 边界。不同 stage 的 loss 组成不同，不能把 Stage 1 的 `loss_total` 和 Stage 2 的 `loss_total` 当成同一种量直接比较。Stage 2 开始后新增了 `loss_exc`、`loss_gpd`、`loss_support` 等项，因此 `loss_total` 的数值尺度会突然变化，这是正常的。

### 7.3 `epoch/train/loss_q` 与 `epoch/val/loss_q`
...
### 7.10 `epoch/*/loss_u_unc` 与 `loss_tail_unc`

`loss_u_unc` 是阈值不确定性辅助项，可以为负。它不应作为主要效果指标。`loss_tail_unc` 当前基本为 0，说明没有触发最小 log-scale 惩罚。

### 7.11 `epoch/*/loss_total`
...
- 重点看 `epoch/val/loss_total` 是否在某个阶段持续恶化。
...
### 7.12 `epoch/lr/group_*`

学习率图窗用于确认训练阶段设置是否正确：

- Stage 1 通常只有一个参数组：encoder + `u_head`。
- Stage 2 有多个参数组：encoder 和 threshold head 用低学习率，tail heads 用原始学习率。
- Stage 3 所有参数统一使用 `finetune_lr`。

如果 loss 在 stage 切换处突然改变，应先看学习率和 loss 组成是否发生了阶段性变化。
...
5. Stage 切换时 loss 组成和学习率同时改变。

### 8.2 需要警惕的波动

以下现象更像训练质量问题：

- `epoch/val/loss_total` 在 Stage 2/3 持续恶化。
- `epoch/val/loss_gpd` 明显退化，而 train 不同步退化。
...
该指标按某个 context feature 分箱。following 中默认使用 `initial_gap`。每个 bin 内比较：
...
| initial_gap bin | n | empirical q95 | predicted q95 mean | abs error |
| --- | ---: | ---: | ---: | ---: |
...
这说明模型学到了“初始间距越小，风险分位越高”的趋势，但各 bin 内均存在低估，尤其最小 gap bin 的高风险尾部低估较明显。
...
5. 模型能学习 initial_gap 与风险分位之间的单调趋势。
...
- 按 `initial_gap` 分箱的 q90/q95 误差。
- 按 `relative_speed_0` 分箱的 q90/q95 误差。
- 按 `ego_vx0` 或 `lead_vx0` 分箱的 q90/q95 误差。
...
an ego-initial coordinate frame, the proposed DeepEVT model predicts a
...
```text
本文将安全关键交通风险预测建模为条件尾部分布估计问题。给定 ego 初始坐标系
下的交通场景初始状态，DeepEVT 模型同时预测条件阈值、超阈概率以及广义帕累托
...
1. `epoch/val/loss_q`：Stage 1 是否稳定下降。
2. `epoch/val/loss_total`：Stage 2/3 是否存在明显退化。
3. `epoch/val/loss_gpd`：尾部 NLL 是否在验证集上恶化。
4. `epoch/val/loss_support`：GPD 支撑域是否稳定。
