...
- `prefix_states`: shape 为 `[prefix_steps, actors, state_features]`，其中 `prefix_steps=1`，两个 actor 分别为 ego 和 target，状态通道为 `x, y, vx, vy, ax, ay`。
- `context_features`: following 场景下为 7 维初始上下文特征。
...
当前 following 模型的 7 个 context 特征为：
...
这些特征只来自窗口第 0 帧。`features.py` 明确禁止以下未来风险字段进入模型输入：
...
这种结构的意义是：模型可以通过 self-attention 学习“ego 速度、前车速度、初始间距、横向偏移、相对速度、加速度”等因素之间的交互，而不是只做简单拼接回归。
...
```text
stage_1/loss_q/val
stage_2/loss_total/val
stage_3/loss_gpd/val
```

这样 `loss_total` 不会把 Stage 1 和 Stage 2/3 画在同一张曲线里。不同 stage 的 `loss_total` 组成不同，只能在同一 stage 内做趋势比较。
...
这些项仍保存在 `training_history.json` 中，但不再默认写入 TensorBoard。它们是训练辅助项，不应作为主要效果指标。TensorBoard 默认聚焦 `loss_total`、`loss_q`、`loss_cal`、`loss_exc`、`loss_gpd` 和 `loss_support`。

### 7.10 `stage_*/loss_total/*`
...
- 重点看 `stage_2/loss_total/val` 或 `stage_3/loss_total/val` 是否在各自阶段内持续恶化。
