# IDM_subset：IDM latent-space 子集模拟

`IDM_subset/` 在 diffusion latent 空间中估计被测车辆在 highD 长尾场景下的闭环高风险概率：
...
100 步；IDM_subset 不再做 rolling reconditioning。

`IDM_subset/` 默认使用经过 held-out 评估的 train/val/test diffusion checkpoint：
...
`IDM_subset/` 使用标准 subset simulation 估计失效率：当第 `level_idx` 层达到失效阈值或
达到最大层数时，
...
`process_highD/` 输出 highD 长尾 scenario conditions 的联合分布；`IDM_subset/` 在该联合分布与
...
`scenario_condition_distribution.npz` 保存已经建模好的 Gaussian-copula 分布参数；`IDM_subset` 只读取
该分布，不在子集模拟阶段重新拟合或从有限 context 行中均匀抽样。

`process_highD` 中基于该分布随机采样、调用扩散模型积分生成轨迹、再与 highD 长尾事件比较，
目的是验证条件扩散模型在给定 scenario condition 下能复现相似测试场景。`IDM_subset` 使用相同的
...
`scenario_condition_distribution.npz`、IDM_subset 可读取的 `tail_contexts.npz`，以及 5000 个 diffusion
generated scenarios。cut-in 的 `tail_contexts.npz` 同时保存 empirical independent tail peaks 和
Gaussian-copula sampled scenario conditions；IDM_subset provider 读取
...
following 和 cut-in 使用不同入口脚本。following IDM_subset 一次性生成 125 步 lead jerk；
...
conda run -n tread python IDM_subset/scripts/run_monte_carlo_following.py
conda run -n tread python IDM_subset/scripts/run_subset_following.py
conda run -n tread python IDM_subset/scripts/play_final_level_following.py --no-gif
```

当前 following IDM_subset 默认使用 `num_samples=3000, p0=0.2, max_levels=8`，并关闭
...
conda run -n tread python IDM_subset/scripts/run_monte_carlo_cutin.py
conda run -n tread python IDM_subset/scripts/run_subset_cutin.py
conda run -n tread python IDM_subset/scripts/play_final_level_cutin.py --no-gif
```

当前 cut-in 默认 MC 使用 10000 个独立样本；IDM_subset 默认使用
...
IDM_subset/src/context_distribution.py
IDM_subset/src/subset_simulation.py
IDM_subset/src/closed_loop_runner.py
IDM_subset/src/latent_evaluator.py
IDM_subset/src/frozen_diffusion_sampler.py
```

`IDM_subset/` 不维护自己的风险公式、EVT 解析、context NPZ 读取或 IDM 参数副本；这些共享逻辑
统一从 `tools/` 读取。闭环回放和 final-level playback 使用 `tools/idm_ego.yaml` 中的
IDM ego 配置，确保 process_highD 与 IDM_subset 的 ego 响应参数一致。
...
- IDM_subset final-level playback 读取 subset simulation 最终层保存的
  ...
  因此它复现的是 IDM_subset 最终层发现的危险闭环样本，而不是重新抽样 diffusion 泛化场景。
