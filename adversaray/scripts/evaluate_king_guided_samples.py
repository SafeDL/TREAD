...
def _plot_closed_loop_histograms(
    ...
    *,
    ...
    panels = (
        (
            "closed-loop risk",
            row_values(prior_rows, "closed_loop_risk"),
            row_values(king_rows, "closed_loop_risk"),
            "risk score",
        ),
        (
            "min gap",
            row_values(prior_rows, "min_gap"),
            row_values(king_rows, "min_gap"),
            "min gap [m]",
        ),
        (
            "min TTC",
            row_values(prior_rows, "min_ttc"),
            row_values(king_rows, "min_ttc"),
            "min TTC [s]",
        ),
        (
            "delta RSS objective",
        ...
        (
            "mean |lead jerk|",
            _trace_series(prior_traces, "lead_jerk", "mean_abs"),
            _trace_series(king_traces, "lead_jerk", "mean_abs"),
            "mean |lead jerk| [m/s^3]",
        ),
        (
            "min ego acceleration",
            _trace_series(prior_traces, "ego_accel", "min"),
            _trace_series(king_traces, "ego_accel", "min"),
            "min ego acceleration [m/s^2]",
        ),
        (
            "mean ego speed",
            _trace_series(prior_traces, "ego_speed", "mean"),
            _trace_series(king_traces, "ego_speed", "mean"),
            "mean ego speed [m/s]",
        ),
    )
    fig, axes = plt.subplots(4, 2, figsize=(12.0, 11.5))
    for ax, (title, prior, king, xlabel) in zip(
        axes.reshape(-1),
        panels,
        strict=True,
    ):
...
def _plot_closed_loop_case(
    ...
    *,
    ...
    ax.plot(
        prior_steps,
        _trace_array(prior_trace, "ego_position"),
        color="C0",
        linestyle="--",
        linewidth=1.6,
        label="ego/prior",
    ...
    ax.plot(
        prior_steps,
        _trace_array(prior_trace, "lead_position"),
        color="C0",
        linewidth=1.8,
        label="lead/prior",
    )
    ax.plot(
        king_steps,
        _trace_array(king_trace, "ego_position"),
        color="C1",
        linestyle="--",
        linewidth=1.6,
        label="ego/king",
    ...
    ax.plot(
        king_steps,
        _trace_array(king_trace, "lead_position"),
        color="C1",
        linewidth=1.8,
        label="lead/king",
    )
...
def _delta_summary(
    ...
    for prior, king in zip(prior_rows, king_rows, strict=True):
        rows.append(
            {
                f"{key}_delta": float(
                    king.get(key, np.nan) - prior.get(key, np.nan)
                )
                for key in keys
            }
        )
...
def _closed_loop_consistency_summary(
    ...
    enough_variance = (
        sample_f.size > 1
        and np.std(sample_f) > 1e-12
        and np.std(closed_f) > 1e-12
    )
    if enough_variance:
...
def _evaluate_case(
    *,
    ...
    king_result.metrics["expert_closed_loop_risk"] = float(
        expert_result.closed_loop_risk
    )
    king_result.metrics["useful_failure_score"] = useful_failure_score
    return (
        case_id,
...
def _log_case(
    ...
    logger.info(
        "Evaluated case %d/%d closed-loop risk %.4f -> %.4f | "
        "gap %.3f -> %.3f | TTC %.3f -> %.3f | RSS %.3f -> %.3f",
        case_id + 1,
        selected,
...
def evaluate_samples(
    *,
    ...
    selected = (
        ...
        else min(total, int(num_contexts))
    )
    if selected <= 0:
    ...
    if num_workers == 1:
        for case_id in range(selected):
            out_id, prior, king, prior_trace, king_trace = _evaluate_case(
                runner=runner,
...
def _case_indices_for_figures(
    ...
    delta = np.asarray(
        [
            king.get("min_gap", np.nan) - prior.get("min_gap", np.nan)
            for prior, king in zip(prior_rows, king_rows, strict=True)
        ],
        dtype=np.float64,
    )
...
def main() -> None:
    ...
    for case_id in _case_indices_for_figures(
        ...
        dataset_index = (
            int(samples["dataset_index"][case_id])
    ...
    logger.info(
        "Saved KING-guided highway-env evaluation summary to %s",
        output_path,
    )
