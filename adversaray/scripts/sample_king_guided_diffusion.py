...
def _sample_summary(arrays: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    return {
        ...
        "naturalness_violation_mean": _array_mean(
            arrays,
            "naturalness_violation",
        ),
        "lambda_naturalness_final_mean": _array_mean(
            arrays,
...
def _pad_actions(
    ...
    *,
    ...
    padded = np.zeros(
        (len(sequences), max_steps, action_dim),
        dtype=np.float32,
    )
...
def _sample_receding_case(
    *,
    ...
    if king_result.prior_actions is None:
        raise RuntimeError(
            "KING rollout did not return reference prior actions"
        )
...
def main() -> None:
    ...
    rename = {
        ...
        "rss_violation_rate": "rss_violation_rate_after",
        "min_relative_rss_margin": "min_relative_rss_margin_after",
        ...
        "prior_rss_violation_rate": "rss_violation_rate_before",
        "prior_min_relative_rss_margin": "min_relative_rss_margin_before",
