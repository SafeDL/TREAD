...
class SAIRLPolicy:
    ...
    def __init__(
        ...
        checkpoint_path: str | Path | None = None,
        *,
        converted_weights_path: str | Path | None = None,
        obs_dim: int = 35,
        action_dim: int = 5,
        ...
        self.converted_weights_path = (
            (
                Path(converted_weights_path)
                if Path(converted_weights_path).is_absolute()
                else (ROOT / Path(converted_weights_path))
            ).resolve()
            if converted_weights_path
            else self.checkpoint_path.with_name(
                self.checkpoint_path.name + ".policy_net.npz"
            )
        )
    ...
    def _load(self) -> None:
        if self.converted_weights_path.exists():
            weights_path = self.converted_weights_path
        elif self.checkpoint_path.suffix == ".npz" and self.checkpoint_path.exists():
            weights_path = self.checkpoint_path
        else:
            weights_path = convert_tensorflow_checkpoint_to_npz(
        ...
        LOGGER.info("Loaded SAIRL policy weights from %s", weights_path)
    ...
    def action_name(self, action: int) -> str:
        return DEFAULT_ACTIONS.get(int(action), "IDLE")
