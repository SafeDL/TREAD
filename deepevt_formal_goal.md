...
DeepEVT following 场景下的 New Round 2 isotonic 结果固化、raw GPD 改进与剩余 6 轮再优化。
...
当前代码与产物已经完成 New Round 1/2 优化，并取得一个比 Round 9 best 更适合作为后续基线的结果。下一阶段的首要要求是：**固化当前 New Round 2 isotonic accepted 结果为新基线，再以该基线为唯一比较对象执行剩余 6 轮有记录的优化**。

本阶段允许继续修改模型结构、训练目标、校准机制、checkpoint selection、评估接口和报告指标；但任何修改都必须证明优于或不劣于当前 New Round 2 accepted baseline。若新方案未能带来净改善，应回滚配置或不接受该轮 checkpoint。
...
2. 后续剩余 6 轮优化前必须记录当前 baseline 指标；
...
## 当前固定基线：New Round 2 Isotonic Accepted
...
raw p_mean = 0.1154；
calibrated p_mean = 0.1847；
calibrated p_mean 与 empirical risk > u 的绝对差距 = 0.0190；
...
q95 calibrated ECE = 0.0067；
q95 calibrated empirical exceed = 0.0433，目标为 0.05；
q95 invalid rate = 0.0；
q95 gap-bin mean abs error = 0.0356；

q95 raw GPD ECE = 0.0422；
q95 raw GPD empirical exceed = 0.0078；

xi_mean = 0.0867；
xi q05/q50/q95 = 0.0570 / 0.0762 / 0.1497；
beta_mean = 0.0974；
beta q05/q50/q95 = 0.0531 / 0.0881 / 0.1808。
```
...
stage = 2；
epoch = 9；
selection_score = 0.6419；
validation empirical exceed = 0.1767；
validation q90 ECE = 0.0002；
validation q95 ECE = 0.0409；
...
在保持 DeepEVT 条件极值建模目标、`risk_score` 定义和 following 场景任务语义不变的前提下，以 New Round 2 isotonic accepted 为固定基线，执行剩余 6 轮优化，使模型从“p_report 条件可靠性已修复、但 raw GPD 仍偏保守”的状态进一步变成“raw GPD 更可信、raw p 更接近真实超阈概率、校准迁移更稳定”的条件尾部分布模型。
...
New Round 1/2 已完成并接受。本阶段后续固定执行剩余 6 轮优化。每轮只围绕一个主假设展开；允许记录失败实验，但只有满足接受条件的结果才能覆盖当前 best 产物。
...
5. 若最终未优于 New Round 2 accepted baseline，则保留当前 New Round 2 accepted baseline。
```
...
原计划固定执行 8 轮优化；New Round 1/2 已完成并接受。后续固定执行剩余 6 轮优化，从 Round 3 开始。
...
5. 与当前 New Round 2 accepted baseline 和上一接受轮比较；
...
以当前 New Round 2 isotonic accepted 为固定基线，执行剩余 6 轮受控优化，在不牺牲 q90、不破坏 q95 calibrated 表现、条件分箱质量和 p reliability 单调性的前提下，重点修复 raw GPD q95 过保守与 raw p 低估，使 DeepEVT following 模型从“后校准与 p_report 可报告”推进到“raw tail、更可信 raw p、条件校准迁移都更稳”的可报告条件尾部分布模型。
