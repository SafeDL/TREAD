...
RESULTS_ROOT = ROOT / "results" / "revision_highd_2026"
...
LOGGER = logging.getLogger("revision_highd_multi_ads")
...
def _frozen_inputs(
    ...
    return {
        "revision_id": "revision_highd_2026",
...
def _run_playbacks(policies: Iterable[str], events: Iterable[str], configs: dict[tuple[str, str], dict[str, Any]]) -> None:
    ...
    for event in events:
        ...
        for candidate in candidates[:2]:
            ...
            module = importlib.import_module(f"{spec.package}.src.final_level_playback")
            ...
            try:
                ...
                manifest = module.replay_final_level(config, spec.config_path(event).parent, expected_event_type=_expected_event(event))
