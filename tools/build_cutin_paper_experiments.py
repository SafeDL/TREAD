...
SOURCE_PATHS = {
    ...
    "natural_lateral_accel_plot": RESULTS
    / "diffusion_natural"
    / "cutin"
    / "natural_prior_plots"
    / "target_lateral_accel_distribution_real_vs_generated.png",
    "natural_trajectory_plot": RESULTS
    / "diffusion_natural"
    / "cutin"
    / "natural_prior_plots"
    / "trajectory_reconstruction_errors.png",
    "natural_lateral_offset_plot": RESULTS
    / "diffusion_natural"
    / "cutin"
    / "natural_prior_plots"
    / "lateral_offset_distribution_real_vs_generated.png",
...
def exp1(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    y = as_float(rows, "y_cutin")
    row: dict[str, Any] = {
        "event_type": "cut_in",
        "num_scored_cutin_candidates": fget(
            event_cache,
            "num_scored_cutin_candidates",
            len(rows) if rows else math.nan,
        ),
        "num_scored_cutin_events": len(rows) if rows else math.nan,
        "num_semantic_cutin_events": fget(event_cache, "num_cutin_contexts"),
        "num_independent_tail_peaks": fget(exposure, "num_independent_tail_peaks"),
        "primary_exposure_label": fget(mileage, "primary_exposure_label", "cut-in all-vehicle"),
        "exposure_denominator": fget(exposure, "exposure_denominator", "all_vehicle_miles"),
        "primary_exposure_miles": fget(exposure, "all_vehicle_miles"),
        "primary_exposure_hours": fget(exposure, "all_vehicle_hours"),
    ...
    if y.size:
        row.update(
            {
                "y_cutin_mean": float(np.mean(y)),
                "y_cutin_std": float(np.std(y, ddof=1)),
                "y_cutin_p50": float(np.quantile(y, 0.50)),
                "y_cutin_p90": float(np.quantile(y, 0.90)),
                "y_cutin_p95": float(np.quantile(y, 0.95)),
                "y_cutin_p99": float(np.quantile(y, 0.99)),
                "y_cutin_max": float(np.max(y)),
    ...
    outputs = write_table("exp1_cutin_event_exposure_stats", [row], force=force)
    if y.size:
        plt = get_pyplot()
        ...
        axes[0].set_xlabel(r"$Y_{\mathrm{cutin}}$")
        ...
        axes[1].set_xlabel(r"$Y_{\mathrm{cutin}}$")
        axes[1].set_ylabel("Empirical CCDF")
        axes[1].legend(frameon=False)
        for ax in axes:
            style_axes(ax)
        fig.tight_layout()
        outputs.extend(save_figure(fig, FIGURES / "exp1_cutin_y_cutin_hist_ccdf.png", force=force))
        plt.close(fig)
...
def exp2(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    outputs = write_table("exp2_cutin_evt_params", [row], force=force)
    ...
    if all(np.isfinite([u, xi, beta, lam, xc])):
        ...
        style_axes(ax)
        fig.tight_layout()
        outputs.extend(save_figure(fig, FIGURES / "exp2_cutin_evt_survival_curve.png", force=force))
        plt.close(fig)
    else:
    ...
    return_levels = evt_summary.get("return_levels", {}) or {}
    if return_levels:
        periods = sorted(
            (float(key[1:]), key)
            for key in return_levels
            if isinstance(key, str) and key.startswith("z")
        )
        if periods:
            xs = [p for p, _ in periods]
            ys = [float(return_levels[key]) for _, key in periods]
            plt = get_pyplot()
            fig, ax = plt.subplots(figsize=(4.4, 3.0))
            ax.plot(xs, ys, marker="o", color=REAL_COLOR, label="EVT return level")
            ci = evt_summary.get("return_level_ci", {}) or {}
            if ci:
                lo = [float(nested(ci, key, "lower")) for _, key in periods]
                hi = [float(nested(ci, key, "upper")) for _, key in periods]
                ax.fill_between(xs, lo, hi, color=REAL_COLOR, alpha=0.18, label="95% CI")
            ax.axhline(xc, color=CRITICAL_COLOR, linestyle="--", label=r"$x_c$")
            ax.set_xscale("log")
            ...
            style_axes(ax)
            fig.tight_layout()
            outputs.extend(save_figure(fig, FIGURES / "exp2_cutin_evt_return_level_curve.png", force=force))
            plt.close(fig)
        status = "generated"
        notes = "return-level curve generated from existing EVT summary return_levels"
    else:
        skip = {"status": "skipped", "reason": "missing return_levels in EVT summary"}
        path = LOGS / "exp2_skipped_return_level_curve.json"
        write_json(path, skip, force=force)
        outputs.append(rel(path))
        status = "generated"
        notes = "return-level curve skipped because existing EVT summary has no return_levels"
    record(
        manifest,
        ...
        for key, value in section.items():
            if any(
                ...
                for token in (
                    ...
                    "lateral_accel_violation_rate",
                    "lateral_jerk_violation_rate",
                    "lateral_displacement_violation_rate",
                )
            ):
                row[key] = value
    return row
...
def exp3(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    outputs = write_table("exp3_cutin_naturalness_summary", [flatten_naturalness(natural)], force=force)
    sources = [
        rel(path)
        for path in (
            SOURCE_PATHS["natural_ax_plot"],
            SOURCE_PATHS["natural_lateral_accel_plot"],
            SOURCE_PATHS["natural_trajectory_plot"],
            SOURCE_PATHS["natural_lateral_offset_plot"],
        )
        if path.exists()
    ]
    record(
...
def exp4(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    outputs = write_table("exp4_cutin_subset_main_results", [row], force=force)
    ...
    if levels:
        ...
        style_axes(ax)
        fig.tight_layout()
        outputs.extend(save_figure(fig, FIGURES / "exp4_cutin_level_score_shift.png", force=force))
        plt.close(fig)
    record(
        manifest,
...
def exp5(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    outputs = write_table("exp5_cutin_ads_vs_highd_intensity", [row], force=force)
    plt = get_pyplot()
    for filename, ads_key, human_key, ylabel in [
        (
            "exp5_cutin_ads_vs_highd_intensity.png",
        ...
        (
            "exp5_cutin_ads_vs_highd_return_period.png",
...
def exp6(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
        ...
        [
            {
                ...
                "score_p95": math.nan,
                "score_max": math.nan,
                "notes": "unavailable in current cutin outputs",
            },
            {
    ...
    outputs = write_table("exp6_cutin_sampling_ablation", rows, force=force)
    ...
    if len(avail) >= 2:
        ...
        for ax in axes:
            ...
            style_axes(ax)
        fig.tight_layout()
        outputs.extend(save_figure(fig, FIGURES / "exp6_cutin_sampling_efficiency.png", force=force))
        plt.close(fig)
...
def exp7(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    rows = [
        {
            "target_name": "raw_y_cutin_threshold",
            "mathematical_definition": "Y_cutin_sim > x_c",
        ...
        },
        {
            ...
            "mathematical_definition": "S_EVT(Y_cutin_sim) > S_EVT(x_c)",
            "requires_evt_model": True,
            "can_map_to_human_return_period": True,
            "available_in_current_outputs": True,
            "estimated_probability_if_available": fget(subset, "probability"),
            "ads_highd_intensity_ratio_if_available": ads_highd_ratio,
            "notes": "monotone-equivalent ordering to Y_cutin_sim > x_c under the fitted GPD score",
        },
        {
            ...
            "notes": (
                "sample-level min_ttc/min_gap exist, but no calibrated cutin surrogate threshold "
                "is defined in current outputs"
            ),
        },
    ]
    outputs = write_table("exp7_cutin_risk_target_ablation", rows, force=force)
    record(
        manifest,
...
def exp8(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    candidates = {
        ...
        for pattern in [
            ...
            "results/**/cutin*normal*/latent_subset_summary.json",
            "results/**/cutin*empirical*/latent_subset_summary.json",
            "results/**/cutin*copula*/latent_subset_summary.json",
        ]
        ...
        rows.append(
            {
                ...
                "notes": "no existing alternative cutin subset summary found; requires a separate explicit config/run",
            }
        )
    ...
    outputs = write_table("exp8_cutin_context_distribution_ablation", rows, force=force)
...
def exp9(manifest: dict[str, Any], data: dict[str, Any], *, force: bool) -> None:
    ...
    row = {
        ...
        "lateral_accel_violation_rate": fget(physical, "lateral_accel_violation_rate"),
        "lateral_jerk_violation_rate": fget(physical, "lateral_jerk_violation_rate"),
        "lateral_displacement_violation_rate": fget(physical, "lateral_displacement_violation_rate"),
    }
    outputs = write_table("exp9_cutin_reliability_diagnostics", [row], force=force)
    plt = get_pyplot()
    fig, axes = plt.subplots(1, 2, figsize=(7.0, 3.0))
    axes[0].bar(
        ["Contexts", "States"],
    ...
    for ax in axes:
        style_axes(ax)
    fig.tight_layout()
    outputs.extend(save_figure(fig, FIGURES / "exp9_cutin_reliability_diagnostics.png", force=force))
    plt.close(fig)
...
def build(force: bool) -> dict[str, Any]:
    ...
    manifest = build_manifest(
        "cut_in",
        "tools/build_cutin_paper_experiments.py",
        ROOT,
        SOURCE_PATHS,
    )
