...
SCRIPT_DEFAULTS: dict[str, Any] = {
    **DEFAULT_HIGHD_LONGITUDINAL_CONFIG,
    ...
    "cutin_independent_tail_peaks_path": (
        ROOT
        / "results"
        / "highd_cutin_tail"
        / "exposure"
...
def _configure_scenario(scenario: str) -> None:
    ...
    if scenario in {"following", "follow"}:
        SCRIPT_DEFAULTS["scenario"] = "following"
        SCRIPT_DEFAULTS["risk_value_key"] = "y_long"
        return
    if scenario in {"cut_in", "cutin", "cut-in"}:
        SCRIPT_DEFAULTS["scenario"] = "cut_in"
        SCRIPT_DEFAULTS["risk_value_key"] = "y_cutin"
        SCRIPT_DEFAULTS["event_context_cache_path"] = SCRIPT_DEFAULTS[
            "cutin_event_context_cache_path"
        ]
        SCRIPT_DEFAULTS["tail_context_path"] = SCRIPT_DEFAULTS[
            "cutin_tail_context_path"
        ]
        SCRIPT_DEFAULTS["independent_tail_peaks_path"] = SCRIPT_DEFAULTS[
            "cutin_independent_tail_peaks_path"
        ]
        SCRIPT_DEFAULTS["evt_model_path"] = SCRIPT_DEFAULTS["cutin_evt_model_path"]
        SCRIPT_DEFAULTS["evt_summary_path"] = SCRIPT_DEFAULTS[
            "cutin_evt_summary_path"
        ]
        return
...
def _apply_evt_scoring(rows: list[dict[str, Any]]) -> tuple[dict[str, Any], Path]:
    ...
    if not evt_model_path.exists():
        raise FileNotFoundError(
            ...
            "process_highD/scripts/fit_following_peak_evt.py first."
        )
    ...
    if str(SCRIPT_DEFAULTS["evt_target_mode"]) == "collision_critical_level":
    ...
    risk_key = str(SCRIPT_DEFAULTS["risk_value_key"])
    ...
    evt_meta = {
        ...
        "evt_target_mode": str(SCRIPT_DEFAULTS["evt_target_mode"]),
        "collision_critical_level": collision_critical_level,
        "risk_value_key": risk_key,
        "scenario": str(SCRIPT_DEFAULTS["scenario"]),
    }
...
def _load_cached_rows() -> list[dict[str, Any]]:
    ...
    if str(SCRIPT_DEFAULTS["scenario"]) == "cut_in":
        rows = load_highd_cutin_event_context_cache(cache_path)
    else:
...
def _load_rows() -> tuple[list[dict[str, Any]], dict[str, Any], str]:
    ...
    return rows, evt_meta, f"{SCRIPT_DEFAULTS['scenario']}_event_context_cache"
...
def _sample_tail_feature_contexts(
    ...
    *,
    ...
    q = float(SCRIPT_DEFAULTS["tail_feature_knn_clip_quantile"])
    q = min(max(q, 0.0), 0.49)
    lower = np.quantile(features, q, axis=0)
    upper = np.quantile(features, 1.0 - q, axis=0)
    bandwidth = max(float(SCRIPT_DEFAULTS["tail_feature_bandwidth"]), 0.0)
...
def _independent_peak_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not peaks_path.exists():
        estimate_script = (
            ...
            if str(SCRIPT_DEFAULTS["scenario"]) == "cut_in"
            else "process_highD/scripts/estimate_following_exposure.py"
        )
        raise FileNotFoundError(
...
def _raw_tail_rows(
    ...
    return [
        row
        for row in rows
        if np.isfinite(float(row[risk_key]))
        and float(row[risk_key]) > tail_threshold_u
    ]
...
def _save_outputs(
    ...
    empirical_context_limit = SCRIPT_DEFAULTS["empirical_context_limit"]
    ...
    context_source = str(SCRIPT_DEFAULTS["context_source"])
    if context_source == "independent_tail_peaks":
        ...
        context_distribution = (
            "uniform over selected highD raw tail "
            f"{SCRIPT_DEFAULTS['scenario']} events"
        )
    else:
    ...
    context_generation_method = str(
        SCRIPT_DEFAULTS["context_generation_method"]
    )
    num_synthetic_contexts = int(SCRIPT_DEFAULTS["num_synthetic_contexts"])
    if context_generation_method == CONTEXT_METHOD_TAIL_FEATURE_KDE_KNN:
        ...
        selected = (
            candidate_rows
            if bool(SCRIPT_DEFAULTS["include_empirical_contexts"])
            else []
        ) + synthetic_rows
        context_distribution = (
        ...
        tail_context_path.with_name("tail_context_summary.json"),
        {
            **evt_meta,
            ...
            "tail_feature_bandwidth": float(
                SCRIPT_DEFAULTS["tail_feature_bandwidth"]
            ),
            "selection_random_seed": int(SCRIPT_DEFAULTS["selection_random_seed"]),
            "scenario": str(SCRIPT_DEFAULTS["scenario"]),
            "risk_value_key": str(SCRIPT_DEFAULTS["risk_value_key"]),
...
def main() -> None:
    ...
    parser.add_argument(
        "--scenario",
        default=str(SCRIPT_DEFAULTS["scenario"]),
        choices=("following", "cut_in"),
        help="Event scenario for tail context selection.",
    )
    args = parser.parse_args()
    _configure_scenario(args.scenario)
