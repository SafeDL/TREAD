...
Loads the EVT model produced by fit_longitudinal_peak_evt.py, and reads
...
def _paths(cfg: dict[str, Any], config_path: Path) -> dict[str, Path]:
    ...
    output_dir_value = exposure_cfg.get(
        "output_dir",
        "../../../results/highd_following_tail/exposure",
    )
    ...
    return {
        ...
        "evt_model": resolve_data_path(
            peak_cfg.get("model_path", SCRIPT_DEFAULTS["evt_model_path"]),
            config_path,
        ),
        "output_dir": resolve_data_path(output_dir_value, config_path),
    }


# ── return-level distance plot helpers (moved from fit_longitudinal_peak_evt) ──
...
def _collision_level_from_config(
    ...
    *,
    ...
    reference_km = peak_cfg.get("collision_critical_reference_km")
    if reference_km is not None:
        reference_km = float(reference_km)
        if reference_km <= 0.0:
            raise ValueError(
                "evt_peak.collision_critical_reference_km must be positive"
            )
    ...
    return (
        float(peak_cfg.get("collision_critical_level", 5.0)),
        {
...
def main() -> None:
    ...
    exposure_cfg = cfg.get("exposure", {})
    peak_cfg = cfg.get("evt_peak", {})
    decluster_cfg = exposure_cfg.get("declustering", {})

    if str(exposure_cfg.get("denominator", "following_ego_miles")) != (
        "following_ego_miles"
    ):
        raise ValueError(
            "Only exposure.denominator='following_ego_miles' is supported"
        )
    if not bool(decluster_cfg.get("enabled", True)):
        raise ValueError(
    ...
    peaks = [
        ...
        if float(peak.get("y_long_max", float("nan"))) > float(model.u)
    ]
    ...
    _write_return_level_plot(
        ...
        distance_min_km=float(peak_cfg.get("distance_plot_min_km", 1.0e3)),
        distance_max_km=float(peak_cfg.get("distance_plot_max_km", 1.0e10)),
        random_seed=int(SCRIPT_DEFAULTS.get("random_seed", 42)),
    )
    ...
    summary = {
        ...
        "declustering_representative": str(
            decluster_cfg.get("representative", "max_y_long")
        ),
        "exposure_denominator": str(
            exposure_cfg.get("denominator", "following_ego_miles")
        ),
    }
