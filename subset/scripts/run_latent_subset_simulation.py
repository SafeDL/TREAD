...
def _mileage_return_period(
    ...
    total_miles = float(exposure.get("following_ego_miles", 0.0))
    total_hours = float(exposure.get("following_ego_hours", 0.0))
    ...
    if str(exposure.get("exposure_denominator", "")) != "following_ego_miles":
    ...
    tail_rate_per_all_vehicle_mile = float(
        exposure.get("tail_peak_rate_per_all_vehicle_mile", 0.0)
    )
    tail_rate_per_all_vehicle_hour = float(
        exposure.get("tail_peak_rate_per_all_vehicle_hour", 0.0)
    )
    ...
    following_periods = _periods(tail_rate_per_mile, tail_rate_per_hour)
    ...
    if target_mode == "collision_critical_level":
        ...
        human_reference = {
            "interpretation": (
                "Human highD following reference at the same EVT safety-critical "
                "level and following-ego exposure denominator."
            ),
    ...
    return {
        ...
        "ads_extreme_risk_intensity_per_mile": following_periods[
            "intensity_per_mile"
        ],
        "ads_safety_critical_intensity_per_mile": following_periods[
            "intensity_per_mile"
        ],
        "ads_return_period_miles": following_periods["return_period_miles"],
        "ads_safety_critical_return_period_miles": following_periods[
            "return_period_miles"
        ],
        "ads_extreme_risk_intensity_per_km": following_periods[
            "intensity_per_km"
        ],
        "ads_safety_critical_intensity_per_km": following_periods[
            "intensity_per_km"
        ],
        "ads_return_period_km": following_periods["return_period_km"],
        "ads_safety_critical_return_period_km": following_periods[
            "return_period_km"
        ],
        "tail_peak_rate_per_hour": tail_rate_per_hour,
        "ads_extreme_risk_intensity_per_hour": following_periods[
            "intensity_per_hour"
        ],
        "ads_safety_critical_intensity_per_hour": following_periods[
            "intensity_per_hour"
        ],
        "ads_return_period_hours": following_periods["return_period_hours"],
        "ads_safety_critical_return_period_hours": following_periods[
            "return_period_hours"
        ],
        ...
        "human_highd_following_reference": human_reference,
        "evt_target_mode": target_mode,
        ...
        "ads_safety_critical_intensity_per_km_at_x_c": (
            following_periods["intensity_per_km"]
            if target_mode == "collision_critical_level"
            else None
        ),
        "ads_safety_critical_return_period_km_at_x_c": (
            following_periods["return_period_km"]
            if target_mode == "collision_critical_level"
            else None
        ),
...
def _log_mileage_return_period(
    ...
    logger.info("=" * 72)
    logger.info(
        "  子集概率 P(Y_long_sim > threshold | tail peak): %.6g",
        probability,
    )
    logger.info(
    ...
    if all_bg:
        ...
        logger.info(
            ...
            float(all_bg.get("following_ego_mile_fraction_of_all_vehicle_miles", 0.0)),
        )
        logger.info(
    ...
    if target_mode == "collision_critical_level" and collision_level is not None:
        ...
        logger.info(
            "  目标模式:          collision_critical_level = %.6g",
            float(collision_level),
        )
        logger.info(
            "  safety-critical 强度(following): %.4g /mile | %.4g /km",
            float(
                mileage.get(
                    "ads_safety_critical_intensity_per_mile_at_x_c",
                    float("nan"),
        ...
        logger.info(
            "  safety-critical 回报周期: %.1f miles | %.1f km",
            float(
                mileage.get(
                    "ads_safety_critical_return_period_miles_at_x_c",
                    float("nan"),
        ...
        human = mileage.get("human_highd_following_reference") or {}
        if human:
