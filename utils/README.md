...
- RSS 基础计算统一从 `utils/rss.py` 引入，仅用于显式 RSS 诊断或历史实验；
  EVT、highD tail 筛选和 subset score 不使用 RSS。
...
- 如果配置 `evt.score_space: evt` 且提供 EVT model，`risk_score` 表示
  ...
  回退为 raw `y_long`。
- 历史或外部 adversarial 实验如需 RSS，可以继续复用 `utils/rss.py` 和
  `torch_plan_proxy_risk()`；这些 RSS 项不在 EVT、subset score 或 highD tail
  筛选中重新计算。
...
EVT 模型由 `process_highD/scripts/fit_longitudinal_peak_evt.py` 生成，保存
...
`fit_longitudinal_peak_evt.py` 和 `select_tail_contexts.py` 必须读取这些缓存；
