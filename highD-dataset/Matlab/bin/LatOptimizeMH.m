...
for i = 1:9
    ...
    [cutinfilter,Result] = CutInFilter(tracks);
    data = [data;Result];
    sumdata = sumdata + length(tracks);
end
...
parfor i=1:length(data)
    temp_Pre_xV = cell2mat(data(i)).Pre_xV;
	temp_Pre_yV = cell2mat(data(i)).Pre_yV;
	temp_Pre_xA = cell2mat(data(i)).Pre_xA;
	temp_Pre_yA = cell2mat(data(i)).Pre_yA;
	temp_duration = cell2mat(data(i)).duration;
    temp_dx = cell2mat(data(i)).dx;
	temp_InitDhw = cell2mat(data(i)).InitDhw;
	temp_InitXVelocity = cell2mat(data(i)).InitXVelocity;
	temp_xVelocity = cell2mat(data(i)).xVelocity;
	temp_InitXAcce = cell2mat(data(i)).InitXAcce;
	temp_xAcceleration = cell2mat(data(i)).xAcceleration;
    Pre_xV = [Pre_xV;temp_Pre_xV];
	Pre_yV = [Pre_yV;temp_Pre_yV];
    Pre_xA = [Pre_xA;temp_Pre_xA];
    Pre_yA = [Pre_yA;temp_Pre_yA];
    duration = [duration;temp_duration];
    dx = [dx;temp_dx];
    InitDhw = [InitDhw;temp_InitDhw];
    InitXVelocity = [InitXVelocity;temp_InitXVelocity];
    xVelocity = [xVelocity;temp_xVelocity];
	InitXAcce = [InitXAcce;temp_InitXAcce];
    xAcceleration = [xAcceleration;temp_xAcceleration];
end
toc
%% 绘制感兴趣的频率直方图，程序入口
% 加载已经保存过的数据，提取highD中有效切入数据，共10维特征
% load('Latdata.mat'); % 全部有效换道数据
% 调用ecdf函数计算xc处的经验分布函数值f_ecd
[f_ecdf_xVelocity, xc_ecdf_xVelocity] = ecdf(Pre_xV);
[f_ecdf_yVelocity, xc_ecdf_yVelocity] = ecdf(Pre_yV);
[f_ecdf_xAcceleration, xc_ecdf_xAcceleration] = ecdf(Pre_xA);
[f_ecdf_yAcceleration, xc_ecdf_yAcceleration] = ecdf(Pre_yA);
[f_ecdf_duration, xc_ecdf_duration] = ecdf(duration);
[f_ecdf_dx, xc_ecdf_dx] = ecdf(dx);
[f_ecdf_InitDhw, xc_ecdf_InitDhw] = ecdf(InitDhw);
[f_ecdf_InitFollowingXVelocity, xc_ecdf_InitFollowingXVelocity] = ecdf(InitXVelocity);
[f_ecdf_FollowingXVelocity, xc_ecdf_FollowingXVelocity] = ecdf(xVelocity);
[f_ecdf_InitFollowingXAcce, xc_ecdf_InitFollowingXAcce] = ecdf(InitXAcce);
[f_ecdf_FollowingXAcce, xc_ecdf_FollowingXAcce] = ecdf(xAcceleration);
%% 频率直方图绘图
clc;
figure(1);
...
[f_ks_xVelocity,xi_xVelocity] = ksdensity(Pre_xV,pts);
...
[f_ks_yVelocity,xi_yVelocity] = ksdensity(Pre_yV,pts);
% 绘制核密度估计图，并设置线条为黑色实线，线宽为3
...
[f_ks_xAcceleration,xi_xAcceleration] = ksdensity(Pre_xA,pts);
% 绘制核密度估计图，并设置线条为黑色实线，线宽为3
...
[f_ks_yAcceleration,xi_yAcceleration] = ksdensity(Pre_yA,pts);
...
ylabel('f(x)');  % 为Y轴加标签
% 调用ksdensity函数进行核密度估计,u1为窗宽
pts = linspace(0.2,150,1000);
[f_ks_InitDhw,xi_InitDhw] = ksdensity(InitDhw,pts,'Support','positive','BoundaryCorrection','reflection');
% 绘制核密度估计图，并设置线条为黑色实线，线宽为3
...
[f_ks_InitFollowingXVelocity,xi_InitFollowingXVelocity] = ksdensity(InitXVelocity,pts);
% 绘制核密度估计图，并设置线条为黑色实线，线宽为3
...
% 调用ksdensity函数进行核密度估计,u1为窗宽
pts = linspace(4,58,1000);
[f_ks_FollowingXVelocity,xi_FollowingXVelocity] = ksdensity(xVelocity,pts,'Support','positive','BoundaryCorrection','reflection');% u1为窗宽
% 绘制核密度估计图，并设置线条为黑色实线，线宽为3
...
[f_ks_InitFollowingXAcce,xi_InitFollowingXAcce] = ksdensity(InitXAcce,pts);% u1为窗宽
% 绘制核密度估计图，并设置线条为黑色实线，线宽为3
...
[f_ks_FollowingXAcce,xi_FollowingXAcce] = ksdensity(xAcceleration,pts);% u1为窗宽
% 绘制核密度估计图，并设置线条为黑色实线，线宽为3
...
InitDhw_sample(1) = unifrnd( 0.02 , 149.88 );
dx_sample(1) = unifrnd( 29.46 , 300 );
xVelocity_sample(1) = unifrnd( 4.36 , 52.96 );
xAcceleration_sample(1) = unifrnd( -5 , 3.03 );
InitFollowingXVelocity_sample(1) = unifrnd( 4.54 , 52.67 );
%% Start sampling
% 定义不同的选择值下的存储结果
...
for i = 0:0.01:40 
    ...
    while t < T % Iterate until we have T samples
        ...
        if (u < alpha_InitDhw) && (InitDhw_star >= 0.02) && (InitDhw_star <= 149.88)
        ...
        if (u < alpha_dx) && (dx_star >= 29.46) && (dx_star <= 300)
...
for i = 0:0.01:5 
    ...
    while t < T % Iterate until we have T samples
        ...
        if (u < alpha) && (xVelocity_star <= 52.96) && (xVelocity_star >= 4.36)
...
for i = 0:0.01:0.4 
    ...
    while t < T % Iterate until we have T samples
        ...
        u = rand;     
        % Do we accept this proposal?
        if (u < alpha) && (xAcceleration_star >= -5) && (xAcceleration_star <= 3.03)
...
for i = 0:0.01:6
    ...
    while t < T % Iterate until we have T samples
        ...
        if (u < alpha_InitFollowingXVelocity) && (InitFollowingXVelocity_star >= 4.54) && (InitFollowingXVelocity_star <= 52.67)
