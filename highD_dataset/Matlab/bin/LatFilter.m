...
% 只提取符合条件的前车切入行驶数据
[cutinfilter,Result] = CutInFilter(tracks); % 前车切入筛选
% [cutinfilter,Result] = cutinfilter(tracks); % 超车换道筛选
% filter 只保留前车切入的跟随数据；Result有效的跟随数据特征
...
argumentsInput.tracks = cutinfilter;
...
argumentsInput.maximumFrame = cutinfilter(end).finalFrame-1; % Maximum frame
